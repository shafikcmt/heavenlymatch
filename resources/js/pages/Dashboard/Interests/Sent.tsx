...
export default function Sent({ interests, counts, currentStatus }: Props) {
  const { t } = useTranslation()
  const [withdrawingId, setWithdrawingId] = useState<number | null>(null)

  const withdraw = (id: number) => {
    setWithdrawingId(id)
    router.delete(route('interests.withdraw', { id }), {
      preserveScroll: true,
  ...
  const emptyMsg = currentStatus === 'pending'
    ? t('interests', 'no_sent')
    : t('interests', `empty_${currentStatus}_sent`)

  return (
    <AppLayout>
      <Head title={t('interests', 'sent_title')} />

      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-slate-900">{t('interests', 'sent_title')}</h1>
          <Link href={route('interests.received')}>
            <Button variant="outline" size="sm">{t('interests', 'go_received')}</Button>
          </Link>
        </div>
        ...
        <div className="flex gap-1 mb-6 rounded-2xl bg-slate-100 p-1">
          {tabs.map(tab => (
            <button
              key={tab.key}
              onClick={() => router.get(route('interests.sent'), { status: tab.key }, { preserveScroll: false })}
              className={cn(
          ...
          <div className="text-center py-20">
            ...
            {currentStatus === 'pending' && (
              <Link href={route('search.index')} className="mt-4 inline-block">
                <Button>{t('interests', 'browse_cta')}</Button>
              </Link>
            )}
          </div>
        ) : (
          <>
            <div className="space-y-4">
              {interests.data.map(interest => {
                ...
                const isWithdrawing = withdrawingId === interest.id
                ...
                return (
                  <div key={interest.id} className="rounded-2xl border border-slate-200 bg-white p-5">
                        ...
                        <div className="flex items-start justify-between gap-2 flex-wrap">
                          <div>
                            ...
                            <p className="text-sm text-slate-500 mt-0.5">
                              {[age ? `${age} yrs` : null, biodata?.district, biodata?.occupation]
                                .filter(Boolean).join(' · ')}
                            </p>
                    ...
                    <div className="mt-4 flex gap-2 flex-wrap">
                      <Link href={route('profile.show', { registrationId: interest.receiver.registration_id })} className="shrink-0">
                      ...
                      {interest.status === 'pending' && (
                        ...
                        >
                          {isWithdrawing ? t('interests', 'withdrawing') : t('interests', 'withdraw')}
                        </Button>
                      )}
